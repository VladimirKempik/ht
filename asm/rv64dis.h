...
#ifndef __RV64_DIS_H__
#define __RV64_DIS_H__

#include "asm.h"
#include "io/types.h"
#include "rv64opc.h"

struct rv64dis_operand {
	int flags;
	const riscv64_operand *op;
	union {
...
struct rv64dis_insn {
	...
	uint32			data;
	int			ops;
	rv64dis_operand		op[8];
};

#define RV_MODE_32 0
#define RV_MODE_64 1

class RV64Disassembler: public Disassembler {
protected:
	char insnstr[256];
    rv64dis_insn insn;
	int mode;
public:
			RV64Disassembler(int mode);
			RV64Disassembler(BuildCtorArg&a): Disassembler(a) {};
